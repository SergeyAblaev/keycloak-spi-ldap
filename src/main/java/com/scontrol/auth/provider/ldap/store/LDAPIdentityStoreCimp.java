...
import com.scontrol.auth.provider.user.LDAPQueryCimp;
 ...
 */
public class LDAPIdentityStoreCimp implements IdentityStore {

    private static final Logger logger = Logger.getLogger(LDAPIdentityStoreCimp.class);
    ...
    public LDAPIdentityStoreCimp(KeycloakSession session, LDAPConfig config) {
        this.config = config;
    ...
    public List<LDAPObject> fetchQueryResults(LDAPQuery identityQuery) {
        ...
        try {
            ...
            StringBuilder filter = createIdentityTypeSearchFilterCimp(identityQuery);
    ...
    public List<LDAPObject> fetchQueryResults(LDAPQueryCimp identityQuery) {
        if (identityQuery.getSorting() != null && !identityQuery.getSorting().isEmpty()) {
            throw new ModelException("LDAP Identity Store does not yet support sorted queries.");
        }
            ...
            for (Condition condition : identityQuery.getConditions()) {
                ...
                if (condition instanceof EqualCondition) {
                    ...
                    if (equalCondition.getParameterName().equalsIgnoreCase(uuidAttrName)) {
                        ...
                        if (search != null) {
                            results.add(populateAttributedTypeCimp(search, identityQuery));
                        }
...
//            if (getConfig().isPagination() && identityQuery.getLimit() > 0) {
//                search = this.operationManager.searchPaginated(baseDN, filter.toString(), identityQuery);
//            } else {    //LDAPConfig.getConnectionUrl()
                    // тут опрашивает провайдеров чтобы выяснить - нет ли такого юзера в домене!
                    //  search = this.operationManager.search(baseDN, filter.toString(), identityQuery.getReturningLdapAttributes(), identityQuery.getSearchScope());
                    //  пока отключу поиск и заменю пустым результатом
//            }

            for (SearchResult result : search) {
                // don't add the branch in subtree search
                if (identityQuery.getSearchScope() != SearchControls.SUBTREE_SCOPE || !baseDN.equals(new LdapName(result.getNameInNamespace()))) {
                    results.add(populateAttributedTypeCimp(result, identityQuery));
                }
            }
        } catch (Exception e) {
            throw new ModelException("Querying of LDAP failed " + identityQuery, e);
    ...
    protected StringBuilder createIdentityTypeSearchFilterCimp(final LDAPQuery identityQuery) {
    ...
    protected StringBuilder createIdentityTypeSearchFilterCimp(final LDAPQueryCimp identityQuery) {
        StringBuilder filter = new StringBuilder();
    ...
    private LDAPObject populateAttributedTypeCimp(SearchResult searchResult, LDAPQueryCimp ldapQuery) {
        Set<String> readOnlyAttrNames = ldapQuery.getReturningReadOnlyLdapAttributes();
        Set<String> lowerCasedAttrNames = new TreeSet<>();
        for (String attrName : ldapQuery.getReturningLdapAttributes()) {
            lowerCasedAttrNames.add(attrName.toLowerCase());
