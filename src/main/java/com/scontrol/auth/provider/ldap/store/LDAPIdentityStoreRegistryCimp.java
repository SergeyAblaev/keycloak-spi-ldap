 ...
 */
public class LDAPIdentityStoreRegistryCimp {

    private static final Logger logger = Logger.getLogger(LDAPIdentityStoreRegistryCimp.class);

    private final Map<String, LDAPConfig> ldapStores = new ConcurrentHashMap<>();

    public LDAPIdentityStoreCimp getLdapStore(KeycloakSession session, ComponentModel ldapModel, Map<ComponentModel, LDAPConfigDecorator> configDecorators) {
     ...
     */
    public static LDAPIdentityStoreCimp createLdapIdentityStore(KeycloakSession session, LDAPConfig cfg) {
        ...
        return new LDAPIdentityStoreCimp(session, cfg);
    }
