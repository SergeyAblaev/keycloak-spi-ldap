 ...
 * @author Shane Bryzak
 */
public class LDAPQueryCimp implements AutoCloseable {

    private static final Logger logger = Logger.getLogger(LDAPQueryCimp.class);

    private final LDAPStorageProviderCimp ldapFedProvider;
    ...
    public LDAPQueryCimp(LDAPStorageProviderCimp ldapProvider) {
        this.ldapFedProvider = ldapProvider;
    }

    public LDAPQueryCimp addWhereCondition(Condition... condition) {
        this.conditions.addAll(Arrays.asList(condition));
        return this;
    }

    public LDAPQueryCimp sortBy(Sort... sorts) {
        this.ordering.addAll(Arrays.asList(sorts));
        return this;
    }

    public LDAPQueryCimp setSearchDn(String searchDn) {
        this.searchDn = LDAPDn.fromString(searchDn).getLdapName();
        return this;
    }

    public LDAPQueryCimp setSearchDn(LdapName searchDn) {
        this.searchDn = searchDn;
        return this;
    }

    public LDAPQueryCimp addObjectClasses(Collection<String> objectClasses) {
        this.objectClasses.addAll(objectClasses);
        return this;
    }

    public LDAPQueryCimp addReturningLdapAttribute(String ldapAttributeName) {
        this.returningLdapAttributes.add(ldapAttributeName);
        return this;
    }

    public LDAPQueryCimp addReturningReadOnlyLdapAttribute(String ldapAttributeName) {
        this.returningReadOnlyLdapAttributes.add(ldapAttributeName.toLowerCase());
        return this;
    }

    public LDAPQueryCimp addMappers(Collection<ComponentModel> mappers) {
        this.mappers.addAll(mappers);
        return this;
    }

    public LDAPQueryCimp setSearchScope(int searchScope) {
    ...
    public List<LDAPObject> getResultList() {
        ...
        for (ComponentModel mapperModel : mappers) { //Todo now: mappers = 0
            LDAPStorageMapperCimp fedMapper = ldapFedProvider.getMapperManager().getMapper(mapperModel);
        ...
        try {
            ...
            logger.errorf("LDAP Query failed", e);
...
//    }
    ...
    public LDAPQueryCimp setLimit(int limit) {
        this.limit = limit;
        return this;
    }

    public LDAPQueryCimp initPagination() throws NamingException {
    ...
    public LDAPStorageProviderCimp getLdapProvider() {
